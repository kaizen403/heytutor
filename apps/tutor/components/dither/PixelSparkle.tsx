...
/*
 * Mirrored from apps/landing/src/components/dither/PixelSparkle.tsx — the two apps share
 * one pixel field, so change both together. Edits here: the "use client"
 * pragma Next needs, and a `cell` prop where the landing hardcodes 3. The
 * default is 3, so the landing's behaviour is unchanged if this is upstreamed.
 */
...
interface PixelSparkleProps {
  ...
  period?: number
  /** Cell size in CSS px. Smaller reads finer; larger reads chunkier. */
  cell?: number
}
...
export default function PixelSparkle({
  ...
  useDitherCanvas(ref, cell, 20, (buf, w, h, _cell, t, reduced) => {
