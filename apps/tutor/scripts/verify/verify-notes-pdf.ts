...
import { renderNotesPdf, type NotesEpoch } from "../../lib/client/exportNotesPdf";
...
assert(reloaded[0]!.images.length === 0 && reloaded[0]!.workLines.length === 2, "turns without a page still keep their work lines");
assert(notesPdfSlideImages(reloaded).join("|") === "img-t", "text-only turns do not become slides");
...
assert(notesPdfSlideImages(grouped).length === 2, "each captured board page is its own slide");
...
assert(fromDb[0]!.workLines.includes("v = 60 cm") && fromDb[0]!.narration.includes("sixty"), "earlier turns still keep work and narration");
assert(fromDb[1]!.images[0] === "img-last" && fromDb[1]!.workLines[0] === "a = g sin θ", "the last snapshot joins its stored turn");
assert(notesPdfSlideImages(fromDb).join("|") === "img-last", "a reloaded board exports only captured slides");

assert(renderNotesPdf([]) === null, "no slides means no PDF");

const PIXEL_PNG =
  "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==";
const slidesDoc = renderNotesPdf([PIXEL_PNG, PIXEL_PNG]);
assert(slidesDoc !== null, "readable board images must produce a PDF");
assert(slidesDoc.getNumberOfPages() === 2, "one PDF page per board slide");
const pdf = slidesDoc.output();
assert(!pdf.includes("Explanation"), "spoken explanation must not appear in the PDF");
assert(!pdf.includes("Board work"), "transcribed board work must not appear in the PDF");
assert(!pdf.includes("Generated by HeyTutor"), "exporter chrome must not appear in the PDF");
assert(!pdf.includes("sixty centimeters"), "subtitles must not be typeset under the slides");

const exporter = readFileSync(resolve(import.meta.dirname, "../../lib/client/exportNotesPdf.ts"), "utf8");
assert(!exporter.includes("Explanation") && !exporter.includes("Board work"), "the exporter must not typeset lesson text");
assert(exporter.includes("addImage"), "the exporter must place board snapshots on the page");
