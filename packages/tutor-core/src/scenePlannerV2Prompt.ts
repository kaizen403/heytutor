...
- representative_slice: {upper: function_curve_id, lower: function_curve_id, atX}. The engine evaluates both functions and requires upper(atX) > lower(atX).
- solid_of_revolution: {profile: function_curve_id, axisY?:0, xMin?, xMax?, samples?}. Produces a closed, symmetric 2D profile for rotation about the horizontal line y=axisY. The function must stay on one side of the axis and may meet it only at domain endpoints.
